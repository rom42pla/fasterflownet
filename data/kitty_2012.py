...
class KITTI2012FlowDataset(Dataset):
    def __init__(self, path, random_crop=False, center_crop=False, zoom=False, rotation=False, horizontal_flip=False,photometric_augmentations=False):
        ...
        for sample_name in self.samples_names:
            sample = {
                "name": sample_name,
                "frame1": join(self.path, "colored_0", f"{sample_name}_10.png"),
                "frame2": join(self.path, "colored_0", f"{sample_name}_11.png")
            }
    ...
    def __getitem__(self, i):
        ...
        for k, v in sample.items():
            if isfile(v) and v.endswith(".png") or v.endswith(".jpg") or v.endswith(".jpeg"):
                # s = torchvision.io.read_image(v).float()/255
                s = None
